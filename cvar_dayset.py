...
class CVARDayset:
    ...
    def var(self, quantile:float):
        return self.total_pnl.quantile(quantile, interpolation=self.interpolation)

    def cvar_index(self, quantile:float):
        return self.pnl_vectors[self.total_pnl <= self.var(quantile)].index
    
    def cvar(self, quantile:float, days:bool=True, assets:bool=True):
        res = self.pnl_vectors.reindex(self.cvar_index(quantile))
        if assets:
            res = res.sum(axis=1)
        if days:
            res = res.mean(axis=0)
    ...
    def _index_sets(self, other, quantile:float):
        sidx = self.cvar_index(quantile)
        oidx = other.cvar_index(quantile)
    ...
    def plot_same_days(self, other, quantile:float, assets:List[str]=None,
        ...
        s = f'Old CVaR({quantile*100}%){contribution_str}:{cvar0}' 
        s = s + f'\nNew Positions on Old CVaR({quantile*100}%) Days:{cvar1}'
        x,y = max(x), min(plt.ylim())*.95
        plt.text(x,y,s, verticalalignment='bottom', horizontalalignment='right')
        plt.tight_layout()
        
        return None
    
    def plot_new_days(self, other, quantile:float, assets:List[str]=None,
                      subplots:Tuple[Subplot, Subplot]=None):
        
        asssets, assets_title, contribution_str = self._assets(other, assets)
        
        heights = other.cvar(quantile, days=False, assets=False)
        heights = heights.reindex(columns=assets).sum(axis=1).sort_values()
        
        x = np.arange(heights.shape[0])
        index_sets = other._index_sets(self, quantile)
        ...
        s = f'New CVaR({quantile*100}%){contribution_str}:{cvar}'
        x,y = max(x), min(plt.ylim())*.95
        plt.text(x,y,s, verticalalignment='bottom', horizontalalignment='right')
        plt.tight_layout()
    ...
    def plot_change(self, other, quantile:float, assets:List[str]=None):
        fig = plt.figure(figsize=(16,8))
        gs = fig.add_gridspec(5,2)
        sub0 = fig.add_subplot(gs[:1,0])
        sub1 = fig.add_subplot(gs[1:,0])
        self.plot_same_days(other, quantile, assets, (sub0, sub1))

        sub2 = fig.add_subplot(gs[:1,1])
        sub3 = fig.add_subplot(gs[1:,1])
        self.plot_new_days(other, quantile, assets, (sub2, sub3))
        
        self._widest_lims([sub1, sub3], 'ylim')
        return None

    def plot_cvar(self, quantile:float, assets:List[str]=None):
        ...
        cvar = f'{int(heights.mean()):,}'
        s = f'CVaR({quantile*100}%){contribution_str}:{cvar}'
        x,y = max(x), min(plt.ylim())*.95
        plt.text(x,y,s, verticalalignment='bottom', horizontalalignment='right')
        plt.tight_layout()
        return None
